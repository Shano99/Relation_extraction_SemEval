...
regex_clean_tags = re.compile("</?[A-Za-z1-2]+>", re.U)
...
class Sentence:  # pylint: disable=too-few-public-methods, too-many-locals, too-many-arguments
    ...
    def __init__(
        ...
        min_tokens: int,
        window_size: int,
        pos_tagger: Any = None, # Can be modified with diff POS tagger
    ):  # noqa: C901
        
        self.relationships = []
        self.tagged_text = None 
        self.entities_regex = re.compile("<[A-Za-z1-2]+>[^<]+</[A-Za-z1-2]+>", re.U)
        entities = list(re.finditer(self.entities_regex, sentence)) # can be modified **
        ...
        if len(entities) >= 2:
            ...
            for ent in entities:
                entity = ent.group()
                e_string = re.findall("<[A-Za-z1-2]+>([^<]+)</[A-Za-z1-2]+>", entity)[0]
                e_type = re.findall("<([A-Za-z1-2]+)", entity)[0]
            ...
            for i in range(len(sorted_keys) - 1):
                ...
                if max_tokens >= distance >= min_tokens and ent1.type == e1_type and ent2.type == e2_type:
                    ...
                    if self.tagged_text is None: # code-change can be done
